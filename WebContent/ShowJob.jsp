...
<div id="wrapper">
  ...
  <div id="inner-wrap">
    ...
    <div id="main">
      <h1>${job.niandu} 學年度 -- ${job.title}</h1>
      <p>${job.comment}</p>
        ...
        <c:forEach var="article" items="${articles}" varStatus="varstatus">
		 <jsp:setProperty name="articleBean" property="id" value="${article.id}" />
          <tr>
            ...
            <td width="10%" style="vertical-align:top"><strong>${article.id}: ${article.title}</strong></td>
          ...
          <tr>
            ...
            <a href="mailto:${article.email}">${articleBean.nianji} ${article.classname}: ${article.author}</a><br/>
            <span style="color:#555555;">
              簡介：<br />
              ${article.comment}<br/><br/>
            <c:forEach var="upfile" items="${article.upfiles}">
              
            檔案：<a href="./Download?articleid=${article.id}">${upfile.filename}</a> <span style=" font-size:9px">(${article.postdate})</span><br />
            </c:forEach>
			<c:if test="${article.id<1410}">            
            檔案：<a href="./Download?articleid=${article.id}">${article.filename}</a> <span style=" font-size:9px">(${article.postdate})</span><br />
            </c:if></span>
            <br/><br/>
            </td>
          </tr>
