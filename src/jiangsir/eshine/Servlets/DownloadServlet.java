...
public class DownloadServlet extends HttpServlet {
	...
	if (articleid < 1410) {
	    ...
	    int upfileid = new ArticleDAO().getArticle(articleid).getUpfiles()
		    .get(0).getId();
	...
	File file = new File(article.getINNER_PATH(),
		article.getINNER_FILENAME());
	if (!file.exists()) {
	    ...
	    exceptionDao.insert_PSTMT(request.getRequestURI(),
		    (String) session.getAttribute("session_account"),
		    request.getRemoteAddr(), "檔案不存在！", exception);
	...
	try {
	    out = response.getOutputStream();
	    response.setContentType("application/octet-stream");
	    String filename = article.getId() + "_" + article.getTitle() + "_"
		    + article.getFilename();
