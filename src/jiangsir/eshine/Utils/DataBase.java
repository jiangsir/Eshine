...
public class DataBase {
	...
	try {
	    ...
	    new ExceptionDAO().insert_PSTMT("DataBase.java", "unknown",
		    "unknown IP", e.getLocalizedMessage(),
		    this.stackTrace(e, sql));
	    e.printStackTrace();
     ...
     * @throws SQLException
     */
    public ResultSet executeQuery(PreparedStatement pstmt, String SQL) {
	long starttime = System.currentTimeMillis();
	ResultSet rs = null;
	try {
    ...
    /**
     * 處理 PSTMT 的 Insert, Update
     * 
     * @param pstmt
     * @param SQL
     * @return
     */
    public int executeUpdate(PreparedStatement pstmt, String SQL) {
	long starttime = System.currentTimeMillis();
	int result = -1;
	try {
