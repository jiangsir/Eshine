...
public class Uploader {
	public static final int MAX_FILESIZE = 5 * 1024 * 1024; // bytes
	...
	public Uploader(HttpServletRequest request, HttpServletResponse response)
		...
		upload.setSizeMax(this.MAX_FILESIZE); // bytes
		try {
			items = upload.parseRequest(request);
		} catch (FileUploadException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			throw new MyException("檔案超過限制(" + this.MAX_FILESIZE / 1024 / 1024
					+ "MB)，請重新處理檔案，避免過大圖檔。");
		}
