...
public class Utils {
	...
	public static void addDenyIP_OLD(String ip) {
		...
		try {
			while ((line = breader.readLine()) != null) {
				if (line.matches(".*Valve className=" + "\"org.apache.catalina.valves.RemoteAddrValve\"" + ".*")) {
	...
	public static boolean isLegalText_Deprecated20081009(String text) {
		...
		if (text == null || text.contains("'") || text.contains("\\") || text.contains(";") || text.contains("--")
				|| text.contains("#")) {
			return false;
		}
		return true;
	...
	public static boolean isMANAGER_IP(String CurrentIP) {
		return Utils.isSubnetwork(ENV.context.getInitParameter("MANAGER_IP"), CurrentIP);
	}
	 ...
	 * 取得指定目錄下的檔案, 不含目錄
	 */
	public static TreeMap<String, Integer> getTestfilenames_Map(String path, String regex) {
		File file = new File(path);
	...
	public static String getBannerfile() {
		...
		String[] VBANNERs = {"VBANNER_1:" + ENV.context.getInitParameter("VBANNER_1"),
